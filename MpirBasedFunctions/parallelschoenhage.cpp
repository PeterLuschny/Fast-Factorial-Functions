...
// Differs only in a single function call from Schoenhage::Factorial.

#include "schoenhage.h"
#include "xmath.h"

void Schoenhage::ParallelFactorial(Xint result, ulong n)
{
    lmp::SetUi(result, 1);
    ...
    for (; iterLen >= 0; iterLen--)
    {
        ...
        if (count > 0)
        {
            Xmath::ParallelProduct(prod, factors, 0, count);
