    ...
    internal class BenchmarkWorker
    {
        ...
        void RelativeRanking(int n)
        {
            if (n < 1000)
            {
                ...
                this.winsole.WriteLine("Please use graeter values (n > 1000) if you want to benchmark.");
            }
            int count = TestParameters.CardSelected;

            var resultList = new Result[count];

            var res = (Result)Candidate.Reference.Performance[n];
            ...
            this.winsole.Write(string.Format(
            ...
            foreach (var result in resultList)
            {
                ...
                if (flag && (r >= 2.05))
                {
                    this.winsole.Write("------------------------" + nl);
                    flag = false;
                }

                this.winsole.Write(string.Format("{0} : {1}\n",
                    result.GetRankAsString2(), result.Cand.Name.Trim()));
            }
            this.winsole.WriteLine(seperatorS + "\n");
        }
        ...
        void UsedTime(int[] benchValues, int benchStart)
        {
            ...
            for (int i = 0; i < Result.RelRankList.Length; i++)
            {
                ...
                this.winsole.WriteRed(cand.Name);

                for (int k = 0; k < benchValues.Length; k++)
                {
                    var res = (Result)cand.Performance[benchValues[k]];
                    this.winsole.WriteRed(res.GetTimeAsString());
                }
                this.winsole.WriteLine();
            }

            this.winsole.WriteLine(seperatorD);
        }
        ...
        void PerformanceProfile(int[] benchValues, int benchStart)
        {
            ...
            for (int i = 0; i < Result.RelRankList.Length; i++)
            {
                ...
                {
                    var res = (Result)cand.Performance[benchValues[k]];
                    this.winsole.WriteRed(res.GetRankAsString());
                }
        ...
        {
            var scale = (int)Math.Pow(10, (int)Math.Log10(Math.Max(100, benchStart)));
        ...
        public void SanityCheck(int length)
        {
            ...
            for (int n = 0; n < length; n++)
            {
                ...
                if ((n % 10) == 0) this.winsole.WriteFlush(" . ");
                if ((n % 150) == 149) this.winsole.WriteLine();
            }
